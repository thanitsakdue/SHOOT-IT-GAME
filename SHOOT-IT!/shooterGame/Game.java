...
public class Game extends JFrame implements ActionListener{
	...
	private final String fileCursorLocked = "images/cursors/unlocked.png",
			fileCursorUnlocked = "images/cursors/locked.png",
	...
	private final String mainbkg = "images/backgrounds/background3.png";
	private final String overbkg = "images/backgrounds/overbkg.png";
	...
	public void setMenuPanel() {
		menuPanel = new JPanel() {
			...
			protected void paintComponent(Graphics g) {
				...
				ImageIcon backgroundImage = new ImageIcon(getClass().getResource(mainbkg));
		...
		innerPanel.setOpaque(false); // Make innerPanel transparent
		...
		// Calculate the position to center the innerPanel horizontally and vertically
		int centerX = (menuPanel.getWidth() - innerPanel.getPreferredSize().width) / 2;
		int centerY = (menuPanel.getHeight() - innerPanel.getPreferredSize().height) / 2;
	
		innerPanel.setBounds(centerX, centerY, innerPanel.getPreferredSize().width, innerPanel.getPreferredSize().height);
   ...
   gameOverPanel = new JPanel() {
			...
			protected void paintComponent(Graphics g) {
				super.paintComponent(g);
				// Load and set the background image
				ImageIcon backgroundImage = new ImageIcon(getClass().getResource(overbkg));
				Image image = backgroundImage.getImage();
				g.drawImage(image, 0, 0, getWidth(), getHeight(), this);
				g.setColor(Color.WHITE);
				g.setFont(Gamefont.getFont(45));
				 g.drawString("Score " + getScore(), getWidth()/2-120, 140);
			}
		};
		
		gameOverPanel.setLayout(null);
		gameOverPanel.setBounds(0, 0, WIDTH, HEIGHT);
		gameOverPanel.setCursor(Game.CURSOR_DEFAULT);

    JPanel innerPanel = new JPanel(new GridLayout(4, 1, 5, 5));
    innerPanel.setPreferredSize(new Dimension(310, 440));
    innerPanel.setOpaque(false);

    restartButton = createButton(new String[] {"images/buttons/restart0.png",
                                               "images/buttons/restart1.png",
                                               "images/buttons/restart2.png"});

    backToMenuButton = createButton(new String[] {"images/buttons/back0.png",
                                                  "images/buttons/back1.png",
                                                  "images/buttons/back2.png"});

    innerPanel.add(restartButton);
    innerPanel.add(backToMenuButton);	
	innerPanel.setSize(310, 440);
	// Calculate the position to center the innerPanel horizontally and vertically
	int centerX = (menuPanel.getWidth() - innerPanel.getPreferredSize().width) / 2;
	int centerY = (menuPanel.getHeight() - innerPanel.getPreferredSize().height) / 2;
	innerPanel.setBounds(centerX, centerY+125, innerPanel.getPreferredSize().width, innerPanel.getPreferredSize().height);
	...
	public void setCreditsPanel() throws IOException {
		...
		if(getClass().getResourceAsStream(fileCredits) == null)
			throw new FileNotFoundException("Due 'credit.txt' Missing ");
		else {
		...
		Thread t = new Thread(new Runnable() {		
			...
			public void run() {
				...
				while(!state.equals(GameState.QUIT)) {
					...
					remove(board);
					repaint();
					add(gameOverPanel); // Display the Game Over panel
					repaint();
					pack();
					setState(GameState.WAIT);
					break;
    ...
    if (e.getSource().equals(playButton)) {
		...
		remove(gameOverPanel);
		repaint();
        setState(GameState.NEW); // Handle Restart button
    } else if (e.getSource().equals(backToMenuButton)) {
        remove(gameOverPanel);
        repaint();
        add(menuPanel); // Handle Back to Menu button
        pack();
        setState(GameState.WAIT);
    }
}
