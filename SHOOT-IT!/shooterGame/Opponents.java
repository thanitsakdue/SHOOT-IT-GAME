...
public abstract class Opponents extends JPanel implements Creature, MouseListener {
	private final int scorePoint = 100;
	
	protected int deadLine;// game will over when Opponents get that position.
	...
	public Opponents(BufferedImage[] frames,int frameLivingLimit, int x, int y) {
	...
	public Opponents(String filePath, int row, int col, int frameLivingLimit, int x, int y) 
			throws IOException {
	...
	//		also it will change manIsDown to true according to Opponents's strength.
	...
	public void move() {
		...
		if(!manIsDown){
			if(x > deadLine)
				...
				x = deadLine;
				GameBoard.isGameOver = true; // game will over when an Opponents reaches to deadLine.
				animationTimer.stop();
			}
